    ...
    <section className="border-b border-[#E6E9EF] bg-[#F6F7FB]">
      <div className="mx-auto max-w-4xl px-6 py-16 sm:px-10 sm:py-20">
        <Link
          ...
          className="inline-flex items-center gap-2 text-sm font-medium text-[#1F5C99] hover:text-[#0B2E59]"
        >
    ...
    <section className="bg-white">
      <div className="mx-auto max-w-4xl px-6 py-16 sm:px-10 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[1fr_1.1fr] lg:gap-16">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-[0.14em] text-[#1F5C99]">
              The challenge
            </h2>
            ...
            <ul className="mt-4 space-y-4">
              {study.approach.map((item) => (
                <li key={item} className="flex gap-3 text-base leading-relaxed text-[#3d4a5c]">
                  <span
                    className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#1F5C99]"
                    aria-hidden="true"
                  />
                  {item}
                </li>
        ...
        <div className="mt-16 rounded-[24px] border border-[#E6E9EF] bg-[#F6F7FB] p-8 sm:p-10">
          ...
          {study.quote && (
            <blockquote className="mt-8 border-t border-[#E6E9EF] pt-8">
              <p className="text-lg font-medium leading-relaxed text-[var(--ink)]">
                “{study.quote.text}”
              </p>
              <footer className="mt-3 text-sm text-[#5a5f6b]">{study.quote.role}</footer>
            </blockquote>
          )}
        ...
        <div className="mt-16 flex flex-col items-start gap-4 rounded-[24px] bg-[#0B1D3A] p-8 sm:flex-row sm:items-center sm:justify-between sm:p-10">
          <div>
            <h2 className="text-xl font-semibold text-white sm:text-2xl">
              Ready to build pipeline that fits how you sell?
            </h2>
            <p className="mt-2 max-w-md text-sm leading-relaxed text-white/75">
              Talk to our marketing team about demand generation, ABM, content, or performance
              campaigns.
            </p>
          </div>
